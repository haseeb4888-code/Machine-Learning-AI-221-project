...
RUN python -m pip install --no-cache-dir --retries 10 --timeout 120 pandas==2.0.0 \
	&& python -c "from src.models.training import TrainingPipeline; from src.models.classification_models import ClassificationModelManager; from src.models.regression_models import RegressionModelManager; p=TrainingPipeline('Old-Version/countries of the world.csv'); p.load_and_preprocess(); p.engineer_features(); p.create_target_variable(); p.prepare_train_test_split(); c=ClassificationModelManager(); c.train_random_forest(p.X_train, p.y_train_clf, p.X_test, p.y_test_clf); c.save_models(); r=RegressionModelManager(); r.train_random_forest_regressor(p.X_train, p.y_train_reg, p.X_test, p.y_test_reg); r.save_models(); print('✓ Baked models into image')"
